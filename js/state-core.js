...
window.AppStateCore = {
    state: {
        ...
        youtube: { isPlaying: false, currentId: null, currentTime: 0, volume: 30 }
    },
    ...
    init: function() {
        ...
        // Слушаем ИЗМЕНЕНИЯ (С исправленной логикой True/False для Blur)
        window.AppEvents.listen('BLUR_TOGGLE', d => { 
            if (d.state === 'on') this.state.blur = true;
            else if (d.state === 'off') this.state.blur = false;
            else this.state.blur = !this.state.blur; // для команд без аргументов
            this.save(); 
        });
        ...
        // Музыка
        window.AppEvents.listen('YT_VISUAL_PLAY', d => { this.state.youtube.currentId = d.id; this.state.youtube.isPlaying = true; this.state.youtube.volume = d.vol; this.save(); });
        window.AppEvents.listen('YT_VISUAL_HIDE', () => { this.state.youtube.isPlaying = false; this.save(); });
